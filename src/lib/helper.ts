...
// Helper function to get embed URL for any video type
export const getVideoEmbedUrl = (project: VideoProject): string | null => {
  if (!project.video_link) return null;

  // Handle Instagram Reels
  if (project.is_instagram_reel && project.video_link.includes("instagram.com/reel/")) {
    const match = project.video_link.match(/instagram\.com\/reel\/([A-Za-z0-9_-]+)/);
    if (match) {
      return `https://www.instagram.com/reel/${match[1]}/embed/`;
    }
  }

  // Handle YouTube videos (including shorts)
  return getYouTubeEmbedUrl(project.video_link);
};
